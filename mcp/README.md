...
snakemake_path: "snakemake"  # 或绝对路径如 "/home/user/miniconda3/envs/atacflow/bin/snakemake"
...
```bash
# 复制整个 mcp/ 目录为新的项目
cp -r ATACFlow/mcp ATACFlow/mcp
```
...
| 新工具名 (蛇形) | 旧工具名 (驼峰) |
|-----------------|-----------------|
| `generate_config_file` | `atacflowGenerateConfigFile` |
...
ATACFlow MCP 使用 **stdio 模式**（标准输入输出），这意味着：
...
如果你的 AI 客户端（如 Claude Desktop）和 ATACFlow MCP 在同一台机器上：
...
如果 ATACFlow MCP 在远程服务器上，使用 SSH 隧道方式：
...
配置完成后，重启 Claude Desktop，你应该能在可用工具列表中看到 ATACFlow 的工具！
...
### 日志位置
所有日志文件存储在：`/home/zj/pipeline/ATACFlow/mcp/logs/mcp/`
...
### 日志格式
```
2026-03-24 23:50:11 - ATACFlowMCP - INFO - === ATACFlow MCP Server 启动 ===
```
...
- `run_atacflow`：配置参数、资源检查、命令执行、进程PID
...
### 数据库概述
使用 SQLite 数据库记录所有 ATACFlow 项目运行信息，数据库文件位于：
`/home/zj/pipeline/ATACFlow/mcp/data/atacflow_runs.db`
...
### 自动功能
- **运行记录**：每次调用 `run_atacflow` 时自动记录到数据库
...
| `data/atacflow_runs.db` | SQLite 数据库，存储项目运行记录 |
...
```bash
# 查看最新的日志文件
cd /home/zj/pipeline/ATACFlow/mcp/logs/mcp/

# 查看最新日志的最后100行
ls -lt /home/zj/pipeline/ATACFlow/mcp/logs/mcp/ | head -2
tail -n 100 /home/zj/pipeline/ATACFlow/mcp/logs/mcp/<最新的日志文件名>

# 实时跟踪日志
tail -f /home/zj/pipeline/ATACFlow/mcp/logs/mcp/<最新的日志文件名>
```
